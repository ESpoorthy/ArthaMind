    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        {ACCOUNTS.map((a) => (
          <div
            key={a.accountId}
            className="bg-gradient-to-br from-blue-700 to-blue-900 rounded-2xl p-6 text-white shadow-lg"
          >
            ...
            <div className="flex justify-between mt-4">
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full ${a.status === 'Active' ? 'bg-green-400/30 text-green-200' : 'bg-red-400/30'}`}
              >
                {a.status}
              </span>
            </div>
          ...
          {txns.map((t) => (
            <div key={t.id} className="flex items-center justify-between px-5 py-3.5">
              <div className="flex items-center gap-3">
                <div
                  className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${t.type === 'credit' ? 'bg-green-100' : 'bg-red-100'}`}
                >
                  {t.type === 'credit' ? (
