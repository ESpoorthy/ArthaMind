...
function getReply(msg: string): { text: string; agent: string } {
  ...
  return {
    text: `Thank you for your query. I'm ArthaMind AI, your personal banking assistant. I can help you with account balances, card management, loans, fixed deposits, and much more. Please ask me anything specific about your banking needs!`,
    agent: 'FAQ Agent',
  };
}
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    {
      id: '0',
      role: 'bot',
      text: `Hello ${user?.name?.split(' ')[0]}! 👋 I'm ArthaMind, your AI banking assistant powered by Gemini 2.5 Flash.\n\nI can help you with:\n• Account balances & transactions\n• Card management\n• Loan eligibility & EMI\n• Fixed deposits\n• Fraud alerts\n\nHow can I help you today?`,
      agent: 'Router Agent',
      time: 'now',
    },
  ]);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
        {msgs.map((m) => (
          <div key={m.id} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${m.role === 'bot' ? 'bg-blue-600' : 'bg-gray-600'}`}
            >
              {m.role === 'bot' ? (
            ...
            <div
              className={`max-w-md ${m.role === 'user' ? 'items-end' : 'items-start'} flex flex-col`}
            >
              {m.agent && <span className="text-xs text-blue-500 mb-1 ml-1">{m.agent}</span>}
              <div
                className={`rounded-2xl px-4 py-3 text-sm whitespace-pre-line ${m.role === 'bot' ? 'bg-white text-gray-800 shadow-sm border border-gray-100' : 'bg-blue-600 text-white'}`}
              >
                {m.text}
              </div>
            </div>
      ...
      <div className="bg-white border-t px-4 pt-3 flex gap-2 overflow-x-auto">
        {suggestions.map((s) => (
          <button
            ...
            className="shrink-0 text-xs bg-blue-50 text-blue-700 border border-blue-200 px-3 py-1.5 rounded-full hover:bg-blue-100 transition"
          >
            {s}
          </button>
        ))}
