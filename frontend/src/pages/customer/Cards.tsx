...
export default function Cards() {
  ...
  const toggle = (id: string) =>
    setCards((cs) =>
      cs.map((c) =>
        c.cardId === id ? { ...c, status: c.status === 'Active' ? 'Blocked' : 'Active' } : c,
      ),
    );
      ...
      <div className="grid grid-cols-2 gap-6">
        {cards.map((card) => (
          <div key={card.cardId} className="space-y-3">
            {/* Visual card */}
            <div
              className={`relative rounded-2xl p-6 shadow-xl overflow-hidden h-48 ${card.type === 'Debit' ? 'bg-gradient-to-br from-blue-700 to-indigo-900' : 'bg-gradient-to-br from-gray-800 to-gray-950'}`}
            >
              {/* Chip */}
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${card.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                  >
                    {card.status}
                  </span>
                </div>
