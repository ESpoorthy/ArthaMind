    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        {FIXED_DEPOSITS.map((fd) => (
          <div
            key={fd.fdId}
            className="bg-gradient-to-br from-purple-600 to-purple-900 rounded-2xl p-6 text-white shadow-lg"
          >
            <div className="flex items-center gap-2 mb-4">
              <PiggyBank className="w-5 h-5 text-purple-200" />
              <span className="text-purple-200 text-sm">Fixed Deposit</span>
              <span className="ml-auto text-xs bg-green-400/30 text-green-200 px-2 py-0.5 rounded-full">
                {fd.status}
              </span>
            </div>
            <p className="text-3xl font-bold mb-1">₹{fd.amount.toLocaleString('en-IN')}</p>
            <p className="text-purple-200 text-sm mb-4">
              {fd.rate}% p.a. · {fd.tenure} months
            </p>
            <div className="flex justify-between bg-white/10 rounded-xl p-3">
              <div>
                <p className="text-xs text-purple-300">Maturity Amount</p>
                <p className="font-bold">₹{fd.maturity.toLocaleString('en-IN')}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-purple-300">Matures On</p>
                <p className="font-bold text-sm">{fd.end}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="bg-purple-50 rounded-xl p-4 flex justify-between items-center">
          <div>
            <p className="text-xs text-purple-600">Maturity Amount</p>
          ...
          <div>
            ...
            <p className="text-xl font-bold text-purple-700">
              +₹{(maturity - amount).toLocaleString('en-IN')}
            </p>
          </div>
          <button className="bg-purple-600 text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-purple-700 transition">
            Open FD
          </button>
        </div>
      </div>
